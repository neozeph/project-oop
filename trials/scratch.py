...
# Function that connects into the database
def connect():
...
def viewwindow():
    ...
    Button(gui, text="Exit Window", font=("candara", 15, "bold"), activebackground="#fffa66", activeforeground="red",
           width=10, command=gui.destroy).pack()

# Function getting the register info
def register():
    a = register_name.get()
    ...
    if c == d and c != "" and len(c) > 5 and a != "" and b != "":
        ...
        if (a == "" or b == "" or c == "" or d == ""):
            ...
            messagebox.showinfo('oops something wrong',
...
def registerGUI():
    ...
    l6 = Label(gui, font=("comic sans ms", 15), bg="#751aff", text="Name").place(x=150, y=195)
    l3 = Label(gui, font=("comic sans ms", 15), bg="#751aff", text="Username").place(x=150, y=243)
    l4 = Label(gui, font=("comic sans ms", 15), bg="#751aff", text="Password").place(x=150, y=293)
    l5 = Label(gui, font=("comic sans ms", 15), bg="#751aff", text="Confirm password").place(x=100, y=342)
    b2 = Button(gui, text="Register", font=("algerian", 19), activebackground="#fffa66", activeforeground="red",
                width=12, command=register).place(x=250, y=400)
    register_name = StringVar()
    e6 = Entry(gui, font=("adobe clean", 15), textvariable=register_name)
    e6.place(x=300, y=200, height=25, width=165)
    register_username = StringVar()
    e3 = Entry(gui, font=("adobe clean", 15), textvariable=register_username)
    e3.place(x=300, y=250, height=25, width=165)
    register_password = StringVar()
    e4 = Entry(gui, font=("adobe clean", 15), textvariable=register_password, show="*")
    e4.place(x=300, y=300, height=25, width=165)
    register_repassword = StringVar()
    e5 = Entry(gui, font=("adobe clean", 15), textvariable=register_repassword, show="*")
    e5.place(x=300, y=350, height=25, width=165)
...
def appwindow():
    ...
    b1 = Button(gui, text="Add Item", font=("georgia", 17), activebackground="#fffa66", activeforeground="red",
                width=10, command=insertitems).place(x=30, y=300)
    b2 = Button(gui, text="View all items", font=("georgia", 17), activebackground="#fffa66", activeforeground="red",
                width=12, command=viewallitems).place(x=110, y=355)
    b3 = Button(gui, text="Delete with id", font=("georgia", 17), activebackground="#fffa66", activeforeground="red",
                width=12, command=deletewithid).place(x=572, y=220)
    b4 = Button(gui, text="Delete all items", font=("georgia", 17), activebackground="#fffa66", activeforeground="red",
                width=15, command=deletealldata).place(x=550, y=280)
    b5 = Button(gui, text="Search", font=("georgia", 17), activebackground="#fffa66", activeforeground="red", width=10,
                command=search_item).place(x=220, y=298)
    b6 = Button(gui, text="Total spent", font=("georgia", 17), activebackground="#fffa66", activeforeground="red",
                width=15, command=sumofitems).place(x=550, y=340)
    b7 = Button(gui, text="Close app", font=("georgia", 17), activebackground="#fffa66", activeforeground="red",
                width=10, command=endpage).place(x=700, y=650)
    l6 = Label(gui, width=60, font=("century", 35), bg="#ff9999", fg="#b32d00", text="EXPENSE  TRACKER").place(x=-450,
                                                                                                               y=0)
...
b3 = Button(root, text="Sign Up", font=("algerian", 19), activebackground="#fffa66", activeforeground="red", width=12, command=registerGUI).place(x=110, y=450)
Label(root, width=60, font=("century", 35), bg="#ff9999", fg="#cc2900", text="EXPENSE  TRACKER").place(x=-400, y=0)
#Label(root, font=("Ink Free", 27), bg="#33cccc", fg="#0000ff", text="R HARSHA").place(x=750, y=590)
b3 = Button(root, text="Exit Window", font=("candara", 15, "bold"), activebackground="#fffa66", activeforeground="red",
            width=10, command=root.destroy).place(x=410, y=630)
